...
func TestVerificationForInvalidWitness(t *testing.T) {
	vk := &VerifyingKey{}
	// grab some example from ZoKrates
	vk.A, _ = NewG2FromStrings(
		[2]string{"0x95aacb3c23ba931cea76b29a19617c347943ecf1002fc0fcf798e0f8d74846c",
	...
	witness[3] = big.NewInt(1) // nonce 1
	witness[4] = big.NewInt(1) // fib 1
	witness[5] = big.NewInt(3) // fib 3 -- invalid here!
...
func TestDirectGT(t *testing.T) {
	scalar := big.NewInt(2)
	G1Point := GetG1Base()
	G2Point := GetG2Base()

	// equal to zero in "in exponentials" equation
	identity := bn256.Miller(G1Point, G2Point)
	identity.Finalize()
	identity.ScalarMult(identity, big.NewInt(0))
	TwoG1 := new(G1).ScalarMult(G1Point, scalar)
	TwoG2 := new(G2).ScalarMult(G2Point, scalar)

	pair := bn256.Miller(new(G1).Neg(G1Point), TwoG2)
	anotherPair := bn256.Miller(TwoG1, G2Point)
	pair = pair.Add(pair, anotherPair)
	pair.Finalize()

	equalToIdentity := bytes.Compare(pair.Marshal(), identity.Marshal()) == 0
	if !equalToIdentity {
		t.Fail()
	}
}

// TestDirectGTwithEntropy One can delay a final exponentiation as much as needed if has access to entropy
func TestDirectGTwithEntropy(t *testing.T) {
	two := big.NewInt(2)
	three := big.NewInt(3)
	entropy := big.NewInt(int64(rand.Uint32()))
	G1Point := GetG1Base()
	G2Point := GetG2Base()
	...
	identity.Finalize()
	identity.ScalarMult(identity, big.NewInt(0))
	TwoG1 := new(G1).ScalarMult(G1Point, two)
	TwoG2 := new(G2).ScalarMult(G2Point, two)

	ThreeG1 := new(G1).ScalarMult(G1Point, three)
	ThreeG2 := new(G2).ScalarMult(G2Point, three)

	pair := bn256.Miller(new(G1).Neg(G1Point), TwoG2)
	pair = pair.Add(pair, bn256.Miller(TwoG1, G2Point))

	anotherPair := bn256.Miller(new(G1).Neg(G1Point), ThreeG2)
	anotherPair = anotherPair.Add(anotherPair, bn256.Miller(ThreeG1, G2Point))

	someCombination := pair.Add(pair, anotherPair.ScalarMult(anotherPair, entropy))

	someCombination.Finalize()

	equalToIdentity := bytes.Compare(someCombination.Marshal(), identity.Marshal()) == 0
	if !equalToIdentity {
		t.Fail()
	}
}

func TestFastVerification(t *testing.T) {
	vk := &VerifyingKey{}
	// grab some example from ZoKrates
	vk.A, _ = NewG2FromStrings(
		[2]string{"0x95aacb3c23ba931cea76b29a19617c347943ecf1002fc0fcf798e0f8d74846c",
	...
	err := agregatedVerification(witness, proof, vk)
	if err != nil {
		t.Fatal(err)
	}
}

func TestFastVerificationForInvalidWitness(t *testing.T) {
	vk := &VerifyingKey{}
	// grab some example from ZoKrates
	vk.A, _ = NewG2FromStrings(
		[2]string{"0x95aacb3c23ba931cea76b29a19617c347943ecf1002fc0fcf798e0f8d74846c",
	...
	// Witness: there is an error here!
	witness := make([]*big.Int, 6)
	witness[0] = big.NewInt(0) // nonce 0
	witness[1] = big.NewInt(1) // fib 1
	witness[2] = big.NewInt(1) // fib 1
...
func BenchmarkNormalVerification(b *testing.B) {
	vk := &VerifyingKey{}
	// grab some example from ZoKrates
	vk.A, _ = NewG2FromStrings(
		[2]string{"0x95aacb3c23ba931cea76b29a19617c347943ecf1002fc0fcf798e0f8d74846c",
	...
	witness[5] = big.NewInt(2) // fib 2
	for i := 0; i < b.N; i++ {
		naiveSplitVerification(witness, proof, vk)
	}
}

func BenchmarkFastVerification(b *testing.B) {
	vk := &VerifyingKey{}
	// grab some example from ZoKrates
	vk.A, _ = NewG2FromStrings(
		[2]string{"0x95aacb3c23ba931cea76b29a19617c347943ecf1002fc0fcf798e0f8d74846c",
