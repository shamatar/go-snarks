...
func naiveSplitVerification(witness Witness, proof *Proof, vk *VerifyingKey) error {
	...
	success := PairingCheck([]*G1{proof.A, temp.Neg(proof.Ap)}, []*G2{vk.A, GetG2Base()})
	if !success {
		return errors.New("First pairing equation has failed")
	}
	// e(vk.B, proof.B) + e(-proof.Bp, G2)
	success = PairingCheck([]*G1{vk.B, temp.Neg(proof.Bp)}, []*G2{proof.B, GetG2Base()})
	if !success {
		return errors.New("Second pairing equation has failed")
	}
	// e(proof.C, vk.C) + e(-proof.Cp, G2)
	success = PairingCheck([]*G1{proof.C, temp.Neg(proof.Cp)}, []*G2{vk.C, GetG2Base()})
	if !success {
	...
	success = PairingCheck([]*G1{u, temp.Neg(proof.H), new(G1).Neg(proof.C)}, []*G2{proof.B, vk.Z, GetG2Base()})
	if !success {
